...
public class BairstowAlgorithm
{
    ...
    private static QuadFactor SolveQuadFactor(Complex[] A, Complex U, Complex V)
    {
        ...
        while (c * c + d * d >= Complex.Tolerance && iteration < ITERMAX)
        {
    ...
    private static QuadraticRoots SolveQuadraticEquation(Complex A, Complex B, Complex C)
    {

        Complex det = B * B - 4 * A * C;
        return new()
        {
            Root1 = (-B + det.SquareRoot()) / (2.0 * A),
            Root2 = -(B / A + (-B + det.SquareRoot()) / (2.0 * A))
        };
